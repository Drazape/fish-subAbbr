...
!!! abstract "Reason: `abbr` limitation"
    ...
    [`abbr`][internal-abbr] identifies an abbreviation as a pair of the [*Sub-Command*][subcommand]{data-preview} and *Base Command*, and an identifier can be assigned to only a single abbreviation.

    Any attempt to create any more with different [*Initial Arguments*][initials]{data-preview} (other than the *Base Command*) will result in overwriting of the older one — because the [*Initial Arguments*][initials]{data-preview} are a concept exclusive to `sub-abbr`.  
    ...
    Therefore, `sub-abbr` internally uses its *Function* switch to parse the `commandline`, letting us create as many abbreviations for the same [*Sub-Command*][subcommand]{data-preview} as we want.
